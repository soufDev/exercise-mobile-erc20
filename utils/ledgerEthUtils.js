...
export const fetchTxs = async (address) => {
    const fetchTxsPage = (address, block) => {
        const url = `http://eth-mainnet.explorers.prod.aws.ledger.fr/blockchain/v3/addresses/${address}/transactions?batch_size=2000&no_token=true${block
            ? '&block_hash=' + block
            : ''}`;
        console.log('GET', url);
 ...
 */
export const getSummary_example = (operations) => {
 ...
 */
export const formatValue = (value, magnitude) => (value / Math.pow(10, magnitude)).toLocaleString();
